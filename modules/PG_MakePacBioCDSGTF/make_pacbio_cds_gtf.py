...
repr_pbs = pd.read_table('./jurkat_orf_aggregated.tsv')['base_acc'].to_list()
...
ranges = pd.read_table('jurkat_refine_orf_calls.tsv')[['pb_acc', 'orf_start', 'orf_end', 'CPM']]
...
pb_gene = pd.read_table('../../results/LR_TranscriptomeSummary/sqanti_isoform_info.tsv')
pb_gene = pd.Series(pb_gene.gene.values, index=pb_gene.pb_acc).to_dict()


with open('pacbio_w_cds.gtf', 'w') as ofile:
